(module
  ...
  (func $average (param $num i32) (param $__offset i32) (result f64)
    ...
    (local $x i32)
    (local $sum f64)
    (local.set $arguments (i32.sub (local.get $__offset) (i32.const 8)))
    (block $@block_1_break
      (loop $@block_1_continue
        (br_if $@block_1_break (i32.ge_s (local.get $x) (local.get $num)))
        (local.set $sum (f64.add (local.get $sum) (block (result f64) (local.set $arguments (i32.add (local.get $arguments) (i32.const 8))) (f64.load align=8 (local.get $arguments)))))
        (local.set $x (i32.add (local.get $x) (i32.const 1)))
        (br $@block_1_continue)))
    (f64.div (local.get $sum) (f64.convert_i32_s (local.get $num))))
